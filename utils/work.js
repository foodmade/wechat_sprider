 ...
 * @type {string}
 */
var basePostdata = '0.9587195974263992&__biz=BIZ%3D%3D&appmsg_type=9&mid=APPMSGID&sn=SN&idx=1&scene=19&title=' +
 ...
 */
function fetchArticleDetailInfo(data,userInfo,callback){
    utils.log('【Start assembling to get the url that the article likes】',config.LOG._LOG_LEVEL_DEBUG);
    utils.log('【The length of the data to be parsed】:' + data.length,config.LOG._LOG_LEVEL_INFO);
    ...
    for (let i=0; i<data.length; i++){
        ...
        utils.httpsPost(options,(msg) => {
            ...
            utils.log('【Do Request like interface success】 ' + JSON.stringify(rspData),config.LOG._LOG_LEVEL_DEBUG);
            callback(rspData,msgid);
        });
    }
}
